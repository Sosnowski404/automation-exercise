...
- Name: api.spec.ts >> /createAccount endpoint tests >> API 11: Create account without company + mobile should return 201
- Location: tests\api.spec.ts:733:9
...
> 738 |         expect(responseBody.responseCode).toBe(201);
  ...
  828 |             password: '',
  829 |         });
  830 |         expect(responseBody.responseCode).toBe(400);
  831 |     });
  832 | 
  833 |     test('Negative: Special characters as email', async ({ request }) => {
  834 |         const responseBody = await createAccount(request, {
  835 |             name: randomName(),
  836 |             email: '!@#$%^&*()',
  837 |             password: randomPassword(),
  838 |         });
```