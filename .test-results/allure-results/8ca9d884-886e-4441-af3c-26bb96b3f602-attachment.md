...
- Name: api.spec.ts >> /createAccount endpoint tests >> API 11: Create account without title should return 400
- Location: tests\api.spec.ts:694:9
...
```ts
  ...
  618 |         });
  619 | 
  620 |         expect(responseBody.responseCode).toBe(400);
  621 |     });
  622 | 
  623 |     test('Negative: Create account with only name should return 400', async ({ request }) => {
  624 |         const responseBody = await createAccount(request, {
  625 |             name: randomName(),
  626 |         });
  627 | 
  628 |         expect(responseBody.responseCode).toBe(400);
  629 |     });
  630 | 
