...
> 518 |         expect(body.responseCode).toBe(200);
  ...
  611 |     });
  612 | 
  613 |     test('Negative: Create account with only name, email, password should return 400', async ({ request }) => {
  614 |         const responseBody = await createAccount(request, {
  615 |             name: randomName(),
  616 |             email: randomEmail(),
  617 |             password: randomPassword(),
  618 |         });
```