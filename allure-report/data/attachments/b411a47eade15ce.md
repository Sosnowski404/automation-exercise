...
- Name: api/create-account.spec.ts >> /createAccount endpoint tests >> Negative: Create account with only password should return 400
- Location: tests/api/create-account.spec.ts:44:9

# Error details

```
Error: Unexpected fields in CreateAccountResponse: [cloudflare_error, detail, error_category, error_code, error_name, footer, instance, owner_action_required, ray_id, retryable, status, timestamp, title, type, what_you_should_do, zone]
```
